...
def test_parent_usage_gated_by_unsaturated_count_delta(db):
    ...
    # Pre-seed a child at the cap. Learn a batch that would otherwise grow usage.
    fwd_root = db.execute(
        "SELECT id FROM trie_nodes WHERE tree = 'F' AND parent_id IS NULL"
    ).fetchone()[0]
    db.execute(
        "INSERT INTO symbols (id, word) VALUES (2, 'HELLO')"
    )
    db.execute(
    ...
    # usage was 0 and count is fully saturated, so usage stays at 0 (no unsaturated portion to add).
    assert after_usage == existing_usage
...
def test_capitalization_skips_low_position_terminal(db):
    """A leading-terminal sequence does not capitalize the next word."""
    # The format CTE only runs over reply output. We verify via a synthetic
    # round-trip: train and ask, but inject ". hello" as a candidate path.
    # Since megahal_reply formats the chosen reply, we drive a deterministic
    # single-candidate model: insert one symbol pair and force the reply.
    # Simpler: probe the format CTE by calling it via a one-off WITH wrapper.
    (out,) = db.execute(
        """
        WITH RECURSIVE
        raw_reply AS (SELECT '. hello world' AS str),
        formatted AS (
            SELECT 1 AS pos,
    ...
    # The leading "." is at position 1, the space is at position 2 (f.pos = 1),
    # so the i>2 guard suppresses the capitalize trigger. "hello" stays lowercase.
